...
DOUBLE PRECISION,DIMENSION(10,20)::WATER_1,WATER_2,WATER_3
DOUBLE PRECISION,DIMENSION(10,20)::WATER_4,WATER_5,WATER_6
DOUBLE PRECISION,DIMENSION(10,20)::WATER_11
...
LOGICAL::JUNK,RANDOM_RUN
INTEGER::FLAG_RANDOM_RUN,FLAG_RANDOM_TIME
INTEGER::random_input,random_input2
...
DOUBLE PRECISION,ALLOCATABLE,DIMENSION(:,:,:)::MELT_DENSITY,SOLID_DENSITY,EX_WATER,SOLID_FRACTION
...
INTEGER::FLAG_MUSH,FLAG_MUSH_INTRUSION,FLAG_TEMPCONT
DOUBLE PRECISION::START_WIDTH,INTRUSION_WIDTH,START_DEPTH,INTRUSION_DEPTH
DOUBLE PRECISION::START_WIDTHZ,INTRUSION_WIDTHZ,INTRUSION_TEMPERATURE
...
OPEN(4234778,FILE='PROFILE_1D_FRAC')
...
OPEN(211013,FILE='EX_WATER',form='unformatted')
OPEN(211013100,FILE='EX_WATER_F',form='unformatted')
OPEN(211014,FILE='SOLID_FRACTION',form='unformatted')
OPEN(211014100,FILE='SOLID_FRACTION_F',form='unformatted')
...
OPEN(110001,FILE='WATER_1')
REWIND(110001)
READ(110001,303) WATER_1

OPEN(110001,FILE='WATER_2')
REWIND(110001)
READ(110001,303) WATER_2

OPEN(110001,FILE='WATER_3')
REWIND(110001)
READ(110001,303) WATER_3

OPEN(110001,FILE='WATER_4')
REWIND(110001)
READ(110001,303) WATER_4

OPEN(110001,FILE='WATER_5')
REWIND(110001)
READ(110001,303) WATER_5

OPEN(110001,FILE='WATER_6')
REWIND(110001)
READ(110001,303) WATER_6

OPEN(110001,FILE='WATER_11')
REWIND(110001)
READ(110001,303) WATER_11
...
!Flag for calculating initial mush
READ(1000,400) DUMMY_CHAR
READ(1000,200) FLAG_MUSH    !Set to 1 for mush calculation

!Intrusion distance from western wall, in km
READ(1000,400) DUMMY_CHAR
READ(1000,300) START_WIDTH

!Intrusion width
READ(1000,400) DUMMY_CHAR
READ(1000,300) INTRUSION_WIDTH

!Intrusion depth, in km
READ(1000,400) DUMMY_CHAR
READ(1000,300) START_DEPTH

!Intrusion thickness, in km
READ(1000,400) DUMMY_CHAR
READ(1000,300) INTRUSION_DEPTH

!Intrusion distance from southern wall, in km
READ(1000,400) DUMMY_CHAR
READ(1000,300) START_WIDTHZ

!Intrusion NS Width, in km
READ(1000,400) DUMMY_CHAR
READ(1000,300) INTRUSION_WIDTHZ

!Intrusion Temperature
READ(1000,400) DUMMY_CHAR
READ(1000,300) INTRUSION_TEMPERATURE

!Flag for intrusion into mush region
READ(1000,400) DUMMY_CHAR
READ(1000,200) FLAG_MUSH_INTRUSION    !Set to 1 for mush calculation

!Flag for enforced mush temperature
READ(1000,400) DUMMY_CHAR
READ(1000,200) FLAG_TEMPCONT   !Set to 1 for temp control (0 to build up mush)

!Flag for random_run
READ(1000,400) DUMMY_CHAR
READ(1000,200) FLAG_RANDOM_RUN   !Set to 1 for random_run (1 for fully random)

!Timestep after which is random (if above this value -- flag above needs to be
!set to 0
READ(1000,400) DUMMY_CHAR
READ(1000,200) FLAG_RANDOM_TIME


DeltaTime=DeltaTime*86400.0*365.0
STEPCOUNT=0
!Convert Frequency Range to Frequency
!Switch for random run - NEW 8_9_21
!initialize
RANDOM_RUN=.FALSE.

IF (FLAG_RANDOM_RUN==1) THEN
RANDOM_RUN=.TRUE.
END IF


IF (RANDOM_RUN==.TRUE.) THEN
CALL RANDOM_SEED
...
IF (FREQUENCY_RANGE==0) THEN
FREQUENCY=100000000
write(100001,*) '0',FREQUENCY
END IF
...
ALLOCATE(EX_WATER(BASE_X,BASE_Y,BASE_Z))
ALLOCATE(SOLID_FRACTION(BASE_X,BASE_Y,BASE_Z))
...
random_input=rand5
FREQUENCYADJUST=Int(ran2(random_input)*FREQUENCY)
...
IF (RANDOM_RUN==.FALSE.) THEN
IF (FLAG_RANDOM_TIME>ZZ) THEN
...
!Risco-Bayo first, RB1-0.0,RB2-200KY-,RB3-400KY,RB4600KY,HUE650 KY
!200 m resolution for now. K is N-S, I E-W, Depth ~5 km
!500 year resolution for now

!Only need two loop section below if tectonically accomodating
!DO II=1,BASE_X
!    DO JJ=1,BASE_Y
!      DO KK=1,BASE_Z
!     MaterialSwitchC(II,JJ,KK)=MaterialSwitch(II,JJ,KK)
...
!     MeltFracDikeN3C(II,JJ,KK)=MeltFracDikeN3(II,JJ,KK)
!     MeltFracCrockN3C(II,JJ,KK)=MeltFracCrockN3(II,JJ,KK)
!     !COMPTC(II,JJ,KK)=COMPT(II,JJ,KK)
!   END DO
!END DO
!END DO

!DO II=(BASE_X/2)+7,(BASE_X/2)+15
!DO KK=(BASE_Z/2)+16,(BASE_Z/2)+31
!DO JJ=BASE_Y-32,BASE_Y-8
!     MaterialSwitch(II,JJ+7,KK)=MaterialSwitchC(II,JJ,KK)
!     MaterialSwitch_B(II,JJ+7,KK)=MaterialSwitchC_B(II,JJ,KK)
!     oldA(II,JJ+7,KK)=oldAC(II,JJ,KK)
!     newA(II,JJ+7,KK)=newAC(II,JJ,KK)
!     Temp(II,JJ+7,KK)=TempC(II,JJ,KK)
!     MeltFracDikeN1(II,JJ+7,KK)=MeltFracDikeN1C(II,JJ,KK)
!     MeltFracCrockN1(II,JJ+7,KK)=MeltFracCrockN1C(II,JJ,KK)
!     MeltFracDikeN2(II,JJ+7,KK)=MeltFracDikeN2C(II,JJ,KK)
!     MeltFracCrockN2(II,JJ+7,KK)=MeltFracCrockN2C(II,JJ,KK)
!     MeltFracDikeN3(II,JJ+7,KK)=MeltFracDikeN3C(II,JJ,KK)
!     MeltFracCrockN3(II,JJ+7,KK)=MeltFracCrockN3C(II,JJ,KK)
!     !COMPT(II,JJ+7,KK)=COMPTC(II,JJ,KK)
!END DO
!END DO
!END DO


write(124323,*) 'I',int((START_WIDTH)/(WIDTH/BASE_X)),int((START_WIDTH+INTRUSION_WIDTH)/(WIDTH/BASE_X))
write(124323,*) 'J',int((HEIGHT-START_DEPTH-INTRUSION_DEPTH)/(HEIGHT/BASE_Y)),int((HEIGHT-START_DEPTH)/(HEIGHT/BASE_Y))
write(124323,*) 'K',int((START_WIDTHZ)/(WIDTHZ/BASE_Z)),int((START_WIDTHZ+INTRUSION_WIDTHZ)/(WIDTHZ/BASE_Z))
write(124323,*) 'T:',INTRUSION_TEMPERATURE
write(124323,*) 'MF:',(INTRUSION_TEMPERATURE-FreezeCrock11)/(MeltCrock11-FreezeCrock11)

!Initial Temperature anomaly
IF (FLAG_TEMPCONT==1) THEN


DO I=int((START_WIDTH)/(WIDTH/BASE_X)),int((START_WIDTH+INTRUSION_WIDTH)/(WIDTH/BASE_X))
 DO J=int((HEIGHT-START_DEPTH-INTRUSION_DEPTH)/(HEIGHT/BASE_Y)),int((HEIGHT-START_DEPTH)/(HEIGHT/BASE_Y))
  DO K=int((START_WIDTHZ)/(WIDTHZ/BASE_Z)),int((START_WIDTHZ+INTRUSION_WIDTHZ)/(WIDTHZ/BASE_Z))
   oldA(I,J,K)=INTRUSION_TEMPERATURE
   Temp(I,J,K)=INTRUSION_TEMPERATURE
   newA(I,J,K)=INTRUSION_TEMPERATURE
MaterialSwitch(I,J,K)=11
   MaterialSwitch_B(I,J,K)=11
!Initial Estimate of melt fraction
   MeltFracCrockN1(I,J,K)=(INTRUSION_TEMPERATURE-FreezeCrock11)/(MeltCrock11-FreezeCrock11)
IF (MeltFracCrockN1(I,J,K)>1.0) MeltFracCrockN1(I,J,K)=1.0
IF (MeltFracCrockN1(I,J,K)<0.0) MeltFracCrockN1(I,J,K)=0.0
  MeltFracCrockN2(I,J,K)=MeltFracCrockN1(I,J,K)
  MeltFracCrockN3(I,J,K)=MeltFracCrockN1(I,J,K)
  !COMPT(I,J,K)=7.0
END DO
END DO
END DO
...
FREQUENCYADJUST1=Int(ran2(random_input)*FREQUENCY)
...
IF (FLAG_MUSH_INTRUSION==1) THEN !Adjust intrusion zone to mush
InjectZone=int(INTRUSION_WIDTH/deltaX)
InjectZoneZ=int(INTRUSION_WIDTHZ/deltaZ)
write(1882881,*) InjectZone,InjectZoneZ
END IF

random_input=rand1
basestart=int((BASE_X/2))+int((.5-ran2(random_input))*REAL(InjectZone))!(int((BASE_X/2)+int(magmagradient/2))*ran2(rand7))!int((BASE_X/2)+(InjectZone/2)-ran2(rand1)*InjectZone)


random_input=rand7
baseend=basestart-5.0+int(10.0*ran2(random_input))!int((BASE_X/2)+(InjectZone/2)-ran2(rand1)*InjectZone)
...
rand2=rand2-1
random_input=rand1
basestartZ=int((BASE_Z/2))+int((.5-ran2(random_input))*REAL(InjectZoneZ))

random_input=rand7
baseendZ=basestartZ-5.0+int(10.0*ran2(random_input))!int((BASE_X/2)+(InjectZone/2)-ran2(rand1)*InjectZone)
...
random_input=rand2
randomL=INT((INT(BASE_CRUST)/(INT(HEIGHT)/BASE_Y))*ran2(random_input))+int((HEIGHT-BASE_CRUST)/(int(HEIGHT)/BASE_Y))   
...
IF (FLAG_MUSH_INTRUSION==1) THEN
 !randomL=BASE_Y-int(START_DEPTH/deltaY) !for now only intrude at one level
randomL=int(START_DEPTH/deltaY)
write(1882881,*) randomL
...
rand4=rand4-1
random_input=rand4
randomX=int(ran2(random_input)*REAL(SILL_P1))
...
rand4=rand4-1
random_input=rand4
randomZ=int(ran2(random_input)*REAL(SILL_P1))
...
IF (FLAG_MUSH_INTRUSION==1) THEN
   ...
   oldA(II,J-SILL_WIDTH+JJ,KK)=MeltCrock11
   newA(II,J-SILL_WIDTH+JJ,KK)=MeltCrock11
   Temp(II,J-SILL_WIDTH+JJ,KK)=MeltCrock11
...
random_input=rand1
basestart=int((BASE_X/2))+int((.5-ran2(random_input))*REAL(InjectZone))

random_input=rand1
random_input2=rand7


 baseend=int((BASE_X/2))+(int((.5-ran2(random_input))*REAL(InjectZone))*FOCUS_FACTOR)-5.0+int(10.0*ran2(random_input2))
...
baseendZ=int((BASE_Z/2))+(int((.5-ran2(random_input))*REAL(InjectZoneZ))*FOCUS_FACTOR)-5.0+int(10.0*ran2(random_input2))
IF (basestartZ==baseendZ) THEN
basestartZ=basestartZ+1
END IF

random_input=rand2
     randomL=INT((HEIGHT-(10000.0*ran2(random_input)))/DY)
...
rand4=rand4-1
random_input=rand4
randomX=int(ran2(random_input)*REAL(SILL_P2))
...
rand4=rand4-1
random_input=rand4
randomZ=int(ran2(random_input)*REAL(SILL_P2))
...
EX_WATER(I,J,K)=((.05*REAL(XVAL1)-x)/.05)*WATER_11(PVAL,XVAL)+((x-.05*REAL(XVAL))/.05)*WATER_11(PVAL,XVAL1)
...
EX_WATER(I,J,K)=((.05*REAL(XVAL1)-x)/.05)*WATER_6(PVAL,XVAL)+((x-.05*REAL(XVAL))/.05)*WATER_6(PVAL,XVAL1)
...
EX_WATER(I,J,K)=((.05*REAL(XVAL1)-x)/.05)*WATER_5(PVAL,XVAL)+((x-.05*REAL(XVAL))/.05)*WATER_5(PVAL,XVAL1)
...
EX_WATER(I,J,K)=((.05*REAL(XVAL1)-x)/.05)*WATER_4(PVAL,XVAL)+((x-.05*REAL(XVAL))/.05)*WATER_4(PVAL,XVAL1)
...
EX_WATER(I,J,K)=((.05*REAL(XVAL1)-x)/.05)*WATER_3(PVAL,XVAL)+((x-.05*REAL(XVAL))/.05)*WATER_3(PVAL,XVAL1)
...
EX_WATER(I,J,K)=((.05*REAL(XVAL1)-x)/.05)*WATER_1(PVAL,XVAL)+((x-.05*REAL(XVAL))/.05)*WATER_1(PVAL,XVAL1)
...
EX_WATER(I,J,K)=((.05*REAL(XVAL1)-x)/.05)*WATER_2(PVAL,XVAL)+((x-.05*REAL(XVAL))/.05)*WATER_2(PVAL,XVAL1)
...
!Set solid fraction based on differences from melt_fraction and ex_water
IF (MaterialSwitch(I,J,K)==2) Then
   x=MeltFracMafic(I,J,K)
ELSE
   x=MeltFracCrust(I,J,K)
...
SOLID_FRACTION(I,J,K)=1.0-x-EX_WATER(I,J,K)

IF (SOLID_FRACTION(I,J,K)>1.0) SOLID_FRACTION(I,J,K)=1.0
IF (SOLID_FRACTION(I,J,K)<0.0) SOLID_FRACTION(I,J,K)=0.0
...
DO J=1,BASE_Y
write(4234777,801) HEIGHT-REAL(J)*deltaY,newA(I,J,K),density(I,J,K),POROSITY(I,J,K)
write(4234778,801) HEIGHT-REAL(J)*deltaY,TOTAL_MELT(I,J,K),SOLID_FRACTION(I,J,K),EX_WATER(I,J,K)
END DO
...
write(211013) EX_WATER
write(211014) SOLID_FRACTION
...
write(211013100) EX_WATER
write(211014100) SOLID_FRACTION
